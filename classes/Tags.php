...
class Tags {
    ...
    public static function getGameTagsByName($con, $name) {
        // Check connection
        if (mysqli_connect_errno($con)) {
            echo "Failed to connect to MySQL: " . mysqli_connect_error();
            return;
        ...
        $sql = "SELECT * FROM games WHERE games.name = '$name'";
        ...
        $row = mysqli_fetch_array($result);
        return array(Tags::getGameTags($con, $row["id"]), $row["id"]);
    }
    ...
    public static function printTags($results, $belong_id) {
        ...
        if (!empty($results)) {
            ...
            while ($row = mysqli_fetch_array($results)) {
                ...
                if (isset($row["email"]) && isset($_SESSION["user"]) && $row["email"] === $_SESSION["user"])
                    echo " author=true";
